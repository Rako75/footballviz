# -*- coding: utf-8 -*-
"""JoueursSimilaires.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1o_NE44EzmMBWIVA_PXrPs-WqY6Vu4dUy
"""
...
@st.cache
def load_data():
    # Remplacez par le chemin réel ou l'URL vers votre fichier CSV
...
# Fonction principale pour trouver les joueurs similaires
def find_similar_players(player_name, league, top_n=10):
    # Filtrer les joueurs selon la ligue sélectionnée
    filtered_df = df[df['Ligue'] == league] if league != "Toutes" else df

    # Vérification des joueurs dans la ligue
    if filtered_df.empty:
        st.warning(f"Aucun joueur trouvé pour la ligue {league}.")
        return None

    # Liste des joueurs disponibles dans la ligue
    list_of_all_players = filtered_df['Joueur'].str.lower().tolist()

    # Recherche du joueur similaire
    ...
    # Index du joueur dans le DataFrame filtré
    player_index = filtered_df[filtered_df['Joueur'].str.lower() == close_match].index[0]

    # Calcul des similarités pour ce joueur
    similarity_matrix = cosine_similarity(filtered_df[selected_features])
    ...
    # Afficher les joueurs similaires
    similar_players = []
...
leagues.insert(0, "Toutes")  # Ajouter une option pour toutes les ligues
...
# Sélectionner le nombre de joueurs similaires
top_n = st.slider("Nombre de joueurs similaires à afficher :", 1, 20, 10)
...
if st.button("Trouver des joueurs similaires"):
    if not player_name:
        ...
        # Colonnes utilisées pour la similarité
        selected_features = [
    ...
    'Courses vers le dernier tiers', 'Courses dans la surface adverse'
        ...
        similar_players = find_similar_players(player_name, selected_league, top_n=top_n)
